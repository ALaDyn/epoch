  ...
  SUBROUTINE mpi_initialise
    ...
    DO ispecies = 1, n_species
      ...
      IF (any_return) THEN
        ...
        ALLOCATE(species_list(ispecies)%net_px_max(1-ng:ny+ng,1-ng:nz+ng))
      END IF
      ...
      bc = species_list(ispecies)%bc_particle(c_bd_z_min)
      IF (bc == c_bc_thermal) THEN
        ALLOCATE(species_list(ispecies)%ext_temp_z_min(1-ng:nx+ng, &
            1-ng:ny+ng,1:3))
      END IF

      bc = species_list(ispecies)%bc_particle(c_bd_z_max)
      IF (bc == c_bc_thermal) THEN
