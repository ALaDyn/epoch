...
CONTAINS
  ...
  SUBROUTINE run_ionisation_loss
    ...
    REAL(num) :: part_x, part_y, part_z, part_ne, part_ne_test, part_v, dE
    REAL(num) :: max_ne, part_Iex_2, part_Iex_C

    ! 1 / (kb * sum(ne) * dx * dy * dz). Sum(ne) is the sum over all solids in a
    ! given cell (an array) not the sum over all cells
    ...
    DO ispecies = 1, n_species
      ...
      DO i_part = 1, part_count
        ...
        CALL calculate_heating(dE, part_x, part_y, part_z, heat_const)
  ...
  SUBROUTINE calculate_heating(dE, part_x, part_y, part_z, heat_const)
    ...
    REAL(num), INTENT(IN) :: heat_const(1-ng:nx+ng, 1-ng:ny+ng, 1-ng:nz+ng)
    INTEGER :: ix, iy,iz
    ...
    iy = FLOOR(part_y * hybrid_idy) + 1
    iz = FLOOR(part_z * hybrid_idz) + 1
#else
    ix = FLOOR(part_x * hybrid_idx + 0.5_num) + 1
    iy = FLOOR(part_y * hybrid_idy + 0.5_num) + 1
    iz = FLOOR(part_z * hybrid_idz + 0.5_num) + 1
#endif
