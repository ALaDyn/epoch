MODULE boosted_frame
  ...
  SUBROUTINE new_simulation_boost
    ...
    IF (use_boosted_frame) THEN
      ...
      IF (rank == 0) &
          PRINT *,'*******************************************'
      ret_err = transform_domain(time_only = .FALSE.)
      any_errors = any_errors .OR. ret_err
      ...
      IF (rank == 0) &
          PRINT *,'*******************************************'
      ret_err = transform_fields()
      any_errors = any_errors .OR. ret_err
      IF (.NOT. ret_err .AND. rank == 0) PRINT *,'Fields OK'
  ...
  FUNCTION get_staggered_field(field, ix, iy, iz, stagger_origin, &
      stagger_destination)
    REAL(num), DIMENSION(1-ng:,1-ng:,1-ng:), INTENT(IN) :: field
    ...
    REAL(num), DIMENSION(-1:1,-1:1,-1:1) :: use_cell

    IF (ALL(stagger_origin == stagger_destination)) THEN
      get_staggered_field = field(ix,iy,iz)
      RETURN
    END IF
    ...
    IF (dstagger(3) == 1) THEN
      use_cell(:,:,-1) = 0.0_num
    ELSE IF (dstagger(3) == 0) THEN
      use_cell(:,:,-1) = 0.0_num
      use_cell(:,:, 1) = 0.0_num
    ELSE IF (dstagger(3) == -1) THEN
      use_cell(:,:, 1) = 0.0_num
    END IF

    get_staggered_field = SUM(field(ix-1:ix+1,iy-1:iy+1,iz-1:iz+1)*use_cell) &
        / SUM(use_cell)
  ...
  FUNCTION get_staggered_fields(ix, iy, iz, stagger_destination)
    INTEGER, INTENT(IN) :: ix, iy, iz
    INTEGER, DIMENSION(c_ndims), INTENT(IN) :: stagger_destination
    REAL(num), DIMENSION(6) :: get_staggered_fields

    get_staggered_fields(1) = get_staggered_field(ex, ix, iy, iz, [1,0,0], &
        stagger_destination)
    get_staggered_fields(2) = get_staggered_field(ey, ix, iy, iz, [0,1,0], &
        stagger_destination)
    get_staggered_fields(3) = get_staggered_field(ez, ix, iy, iz, [0,0,1], &
        stagger_destination)

    get_staggered_fields(4) = get_staggered_field(bx, ix, iy, iz, [0,1,1], &
        stagger_destination)
    get_staggered_fields(5) = get_staggered_field(by, ix, iy, iz, [1,0,1], &
        stagger_destination)
    get_staggered_fields(6) = get_staggered_field(bz, ix, iy, iz, [1,1,0], &
        stagger_destination)
  ...
  FUNCTION transform_fields() RESULT(any_errors)
    ...
    ALLOCATE(exn(1-ng:nx+ng, 1-ng:ny+ng, 1-ng:nz+ng))
    ALLOCATE(eyn(1-ng:nx+ng, 1-ng:ny+ng, 1-ng:nz+ng))
    ALLOCATE(ezn(1-ng:nx+ng, 1-ng:ny+ng, 1-ng:nz+ng))
    ALLOCATE(bxn(1-ng:nx+ng, 1-ng:ny+ng, 1-ng:nz+ng))
    ALLOCATE(byn(1-ng:nx+ng, 1-ng:ny+ng, 1-ng:nz+ng))
    ALLOCATE(bzn(1-ng:nx+ng, 1-ng:ny+ng, 1-ng:nz+ng))

    DO iz = 1, nz
      DO iy = 1, ny
        DO ix = 1, nx
          fields_in = get_staggered_fields(ix, iy, iz, [1, 0 ,0])
          CALL transform_em_fields(global_boost_info, fields_in, fields_out)
          exn(ix,iy,iz) = fields_out(1)

          fields_in = get_staggered_fields(ix, iy, iz, [0, 1 ,0])
          CALL transform_em_fields(global_boost_info, fields_in, fields_out)
          eyn(ix,iy,iz) = fields_out(2)

          fields_in = get_staggered_fields(ix, iy, iz, [0, 0 ,1])
          CALL transform_em_fields(global_boost_info, fields_in, fields_out)
          ezn(ix,iy,iz) = fields_out(3)

          fields_in = get_staggered_fields(ix, iy, iz, [0, 1 ,1])
          CALL transform_em_fields(global_boost_info, fields_in, fields_out)
          bxn(ix,iy,iz) = fields_out(4)

          fields_in = get_staggered_fields(ix, iy, iz, [1, 0 ,1])
          CALL transform_em_fields(global_boost_info, fields_in, fields_out)
          byn(ix,iy,iz) = fields_out(5)

          fields_in = get_staggered_fields(ix, iy, iz, [1, 1 ,0])
          CALL transform_em_fields(global_boost_info, fields_in, fields_out)
          bzn(ix,iy,iz) = fields_out(6)
        END DO
      END DO
    END DO
  ...
  FUNCTION transform_species(inverse) RESULT(any_errors)
    ...
    DO ispecies = 1, n_species
      ...
      DO WHILE (ASSOCIATED(current))
#ifdef PHOTONS
        IF (species_in%species_type /= c_species_id_photon) THEN
#endif
          retval = transform_particle(current,mass = species_in%mass, &
              inverse = inverse)
#ifdef PHOTONS
        ELSE
          retval = transform_particle(current,energy = current%energy, &
              inverse = inverse)
        END IF
#endif
  ...
  FUNCTION transform_particle(current, mass, energy, &
      inverse) RESULT(any_errors)
  ...
  FUNCTION transform_laser(laser) RESULT(any_errors)
    ...
    IF (.NOT. laser%use_omega_function .AND. &
      ...
      DO j = LBOUND(laser%omega,2), UBOUND(laser%omega,2)
        DO i = LBOUND(laser%omega,1), UBOUND(laser%omega,1)
          IF (laser%omega_func_type == c_of_k) THEN
            kx = laser%k(i, j, 1)
          ELSE
            kx = laser%omega(i, j)/c * laser%kx_mult
          END IF

          laser%omega(i, j) = transform_frequency(global_boost_info, &
              laser%omega(i, j), kx, k_boost = kboost)
          IF (kboost < minkboost) minkboost=kboost
          IF (laser%omega_func_type == c_of_k) THEN
            laser%k(i, j, 1) = kboost
          END IF
        END DO
      END DO
  ...
  FUNCTION transform_io() RESULT(any_errors)
    ...
    DO iprefix = 1, nprefix
      ...
      ALLOCATE(prefix_boosts(iprefix)%field_lists(1-ng:nx+ng, 1-ng:ny+ng, &
          1-ng:nz+ng, 6, n_recorders))
