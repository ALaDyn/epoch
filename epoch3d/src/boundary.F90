...
CONTAINS
  ...
  SUBROUTINE do_field_mpi_with_lengths_logic(field, ng, nx_local, ny_local, &
      nz_local)

    INTEGER, INTENT(IN) :: ng
    LOGICAL, DIMENSION(1-ng:,1-ng:,1-ng:), INTENT(INOUT) :: field
    INTEGER, INTENT(IN) :: nx_local, ny_local, nz_local
    INTEGER, DIMENSION(c_ndims) :: sizes, subsizes, starts
    INTEGER :: subarray, basetype, sz, szmax, i, j, k, n
  ...
  SUBROUTINE particle_reflection_bcs(array, ng, flip_direction, species)
    ...
    n = n + 1
    bc = bc_species(n)
    IF (y_min_boundary .AND. (bc == c_bc_reflect .OR. bc == c_bc_tnsa)) THEN
      IF (flip_dir == (n-1)/2 + 1) THEN
        ! Currents get reversed in the direction of the boundary
        DO i = 1, ng-1
          array(:,i,:) = array(:,i,:) - array(:,-i,:)
    ...
    n = n + 1
    bc = bc_species(n)
    IF (y_max_boundary .AND. (bc == c_bc_reflect .OR. bc == c_bc_tnsa)) THEN
      IF (flip_dir == (n-1)/2 + 1) THEN
        ! Currents get reversed in the direction of the boundary
        DO i = 1, ng
          array(:,nn-i,:) = array(:,nn-i,:) - array(:,nn+i,:)
    ...
    n = n + 1
    bc = bc_species(n)
    IF (z_min_boundary .AND. (bc == c_bc_reflect .OR. bc == c_bc_tnsa)) THEN
      IF (flip_dir == (n-1)/2 + 1) THEN
        ! Currents get reversed in the direction of the boundary
        DO i = 1, ng-1
          array(:,:,i) = array(:,:,i) - array(:,:,-i)
    ...
    n = n + 1
    bc = bc_species(n)
    IF (z_max_boundary .AND. (bc == c_bc_reflect .OR. bc == c_bc_tnsa)) THEN
      IF (flip_dir == (n-1)/2 + 1) THEN
        ! Currents get reversed in the direction of the boundary
        DO i = 1, ng
          array(:,:,nn-i) = array(:,:,nn-i) - array(:,:,nn+i)
            ...
            IF (x_max_boundary) THEN
              ...
              IF (bc == c_bc_reflect .OR. bc == c_bc_tnsa) THEN
                ...
                IF (bc == c_bc_tnsa) THEN
                  CALL tnsa_part_escape(cur, out_of_bounds, ispecies)
                  IF (.NOT. out_of_bounds) THEN
                    CALL tnsa_part_reflect(cur, c_bd_x_max, ispecies)
                  END IF
                END IF
              ELSE IF (bc == c_bc_periodic) THEN
                xbd = 1
            ...
            IF (y_min_boundary) THEN
              ...
              IF (bc == c_bc_reflect .OR. bc == c_bc_tnsa) THEN
                ...
                IF (bc == c_bc_tnsa) THEN
                  CALL tnsa_part_escape(cur, out_of_bounds, ispecies)
                  IF (.NOT. out_of_bounds) THEN
                    CALL tnsa_part_reflect(cur, c_bd_y_min, ispecies)
                  END IF
                END IF
              ELSE IF (bc == c_bc_periodic) THEN
                ybd = -1
            ...
            IF (y_max_boundary) THEN
              ...
              IF (bc == c_bc_reflect .OR. bc == c_bc_tnsa) THEN
                ...
                IF (bc == c_bc_tnsa) THEN
                  CALL tnsa_part_escape(cur, out_of_bounds, ispecies)
                  IF (.NOT. out_of_bounds) THEN
                    CALL tnsa_part_reflect(cur, c_bd_y_max, ispecies)
                  END IF
                END IF
              ELSE IF (bc == c_bc_periodic) THEN
                ybd = 1
            ...
            IF (z_min_boundary) THEN
              ...
              IF (bc == c_bc_reflect .OR. bc == c_bc_tnsa) THEN
                ...
                IF (bc == c_bc_tnsa) THEN
                  CALL tnsa_part_escape(cur, out_of_bounds, ispecies)
                  IF (.NOT. out_of_bounds) THEN
                    CALL tnsa_part_reflect(cur, c_bd_z_min, ispecies)
                  END IF
                END IF
              ELSE IF (bc == c_bc_periodic) THEN
                zbd = -1
            ...
            IF (z_max_boundary) THEN
              ...
              IF (bc == c_bc_reflect .OR. bc == c_bc_tnsa) THEN
                ...
                IF (bc == c_bc_tnsa) THEN
                  CALL tnsa_part_escape(cur, out_of_bounds, ispecies)
                  IF (.NOT. out_of_bounds) THEN
                    CALL tnsa_part_reflect(cur, c_bd_z_max, ispecies)
                  END IF
                END IF
              ELSE IF (bc == c_bc_periodic) THEN
                zbd = 1
