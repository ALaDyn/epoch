  ...
  SUBROUTINE redistribute_fields(new_domain)
    ...
    DO WHILE(ASSOCIATED(injector_next))
      ...
      IF (ASSOCIATED(injector%drift_perp)) THEN
        CALL remap_field_slice(c_dir_x, injector%drift_perp, temp_slice)
        DEALLOCATE(injector%drift_perp)
        ALLOCATE(injector%drift_perp(1-ng:ny_new+ng))
        injector%drift_perp = temp_slice
      END IF
    ...
    DO ispecies = 1, n_species
      ...
      IF (bc == c_bc_return) THEN
        ...
        ALLOCATE(species_list(ispecies)%ext_drift_x_min(1-ng:ny_new+ng))
        species_list(ispecies)%ext_drift_x_min = temp_slice
      ...
      IF (bc == c_bc_return) THEN
        ...
        ALLOCATE(species_list(ispecies)%ext_drift_x_max(1-ng:ny_new+ng))
        species_list(ispecies)%ext_drift_x_max = temp_slice
