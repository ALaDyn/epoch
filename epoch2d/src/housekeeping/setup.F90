  ...
  SUBROUTINE read_return_injectors(sdf_handle, block_id_in, ndims, &
    ...
    IF (str_cmp(block_id_in(1:lstr), 'return_injector') &
      ...
      IF (ndims /= 1 .OR. dims(1) /= ny_global) THEN
        ...
        PRINT*, 'Return boundaries may not restart correctly'
      END IF
      ...
      IF (boundary == c_bd_x_min) THEN
        curr_species%ext_drift_x_min(1:ny) = values(ny_global_min:ny_global_max)
      ELSE IF(boundary == c_bd_x_max) THEN
        curr_species%ext_drift_x_max(1:ny) = values(ny_global_min:ny_global_max)
      END IF
