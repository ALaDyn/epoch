...
CONTAINS
  ...
  SUBROUTINE setup_hy_elastic_scatter
  ...
  END SUBROUTINE setup_hy_elastic_scatter
  ...
  SUBROUTINE setup_urban_cache(i_sol)
    ...
    IF (solid_array(i_sol)%hybrid_Z < Z_tab(2)) THEN
      iZ2 = 2
    ELSE IF (solid_array(i_sol)%hybrid_Z >= Z_tab(14)) THEN
      iZ2 = 15
    ELSE
      DO iZ2 = 3,14
        IF (Z_tab(iZ2) > solid_array(i_sol)%hybrid_Z) THEN
          EXIT
        END IF
      END DO
    END IF
    ...
    ! Interpolate in Z²
    IF (solid_array(i_sol)%hybrid_Z < Z_tab(1)) THEN
      ! Z < 4
      frac = (solid_array(i_sol)%hybrid_Z / Z_tab(1))**2
    ...
    ELSE IF (solid_array(i_sol)%hybrid_Z > Z_tab(15)) THEN
      ! Z > 82
      frac = (solid_array(i_sol)%hybrid_Z / Z_tab(15))**2
    ...
    ELSE
      ! 4 <= Z <= 82
      frac = (solid_array(i_sol)%hybrid_Z**2 - Z_tab(iZ1)**2) &
      ...
      DO ix = 1-ng,nx+ng
        ...
        DO isol = 1,solid_count
          ...
          urb_Zeff(ix,iy) = urb_Zeff(ix,iy) &
              + solid_array(isol)%ion_density(ix,iy)*solid_array(isol)%hybrid_Z
  ...
  FUNCTION compute_lambda1(KE)
    ...
    IF (KE < KE_10MeV) THEN
      ...
      CALL hy_grid_centred_var_at_particle(part_x, part_y, part_iZ23, urb_iZ23)
      ...
      DO isol = 1, solid_count
        ...
        sigma = sigma + part_ni * solid_array(isol)%hybrid_Z**2 * sigma_2 / corr
      END DO
  ...
  FUNCTION compute_theta0(KE, step_size)
    ...
    CALL hy_grid_centred_var_at_particle(part_x, part_y, part_rad, urb_rad_len)
    ...
    IF (species_list(ispecies)%species_type == c_species_id_positron) THEN
      ...
      CALL hy_grid_centred_var_at_particle(part_x, part_y, part_Zeff, urb_Zeff)
    ...
    CALL hy_grid_centred_var_at_particle(part_x, part_y, part_th1, urb_theta1)
    CALL hy_grid_centred_var_at_particle(part_x, part_y, part_th2, urb_theta2)
