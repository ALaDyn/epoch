...
CONTAINS
  ...
  SUBROUTINE run_ionisation_loss
    ...
    REAL(num) :: part_x, part_ne, part_ne_test, part_v, dE
    REAL(num) :: max_ne, part_Iex_2, part_Iex_C

    ! 1 / (kb * sum(ne) * dx) - last bit is per unit volume, but dy and dz are
    ! 1m in epoch1d. Sum(ne) is the sum over all solids in a given cell (an
    ! array), not the sum over all cells
    ...
    DO ispecies = 1, n_species
      ...
      DO i_part = 1, part_count
        ...
        CALL calculate_heating(dE, part_x, heat_const)
  ...
  SUBROUTINE calculate_heating(dE, part_x, heat_const)
    ...
    REAL(num), INTENT(IN) :: heat_const(1-ng:nx+ng)
    INTEGER :: ix
    REAL(num) :: part_C, part_heat_const, delta_Tb

    CALL hy_grid_centred_var_at_particle(part_x, part_heat_const, heat_const)
