MODULE boosted_frame
  ...
  FUNCTION get_staggered_field(field, ix, stagger_origin, stagger_destination)
    REAL(num), DIMENSION(1-ng), INTENT(IN) :: field
    ...
    REAL(num), DIMENSION(-1:1) :: use_cell

    IF (ALL(stagger_origin == stagger_destination)) THEN
      get_staggered_field = field(ix)
      RETURN
    END IF
    ...
    get_staggered_field = SUM(field(ix-1:ix+1)*use_cell) / SUM(use_cell)

  END FUNCTION get_staggered_field



  FUNCTION get_staggered_fields(ix, stagger_destination)
    INTEGER, INTENT(IN) :: ix
    INTEGER, DIMENSION(c_ndims), INTENT(IN) :: stagger_destination
    REAL(num), DIMENSION(6) :: get_staggered_fields

    get_staggered_fields(1) = get_staggered_field(ex, ix, [1], &
        stagger_destination)
    get_staggered_fields(2) = get_staggered_field(ey, ix, [0], &
        stagger_destination)
    get_staggered_fields(3) = get_staggered_field(ez, ix, [0], &
        stagger_destination)

    get_staggered_fields(4) = get_staggered_field(bx, ix, [0], &
        stagger_destination)
    get_staggered_fields(5) = get_staggered_field(by, ix, [1], &
        stagger_destination)
    get_staggered_fields(6) = get_staggered_field(bz, ix, [1], &
        stagger_destination)
  ...
  FUNCTION transform_fields() RESULT(any_errors)
    ...
    ALLOCATE(exn(1-ng:nx+ng))
    ALLOCATE(eyn(1-ng:nx+ng))
    ALLOCATE(ezn(1-ng:nx+ng))
    ALLOCATE(bxn(1-ng:nx+ng))
    ALLOCATE(byn(1-ng:nx+ng))
    ALLOCATE(bzn(1-ng:nx+ng))

    DO ix = 1, nx
      fields_in = get_staggered_fields(ix, [1])
      CALL transform_em_fields(global_boost_info, fields_in, fields_out)
      exn(ix) = fields_out(1)

      fields_in = get_staggered_fields(ix, [0])
      CALL transform_em_fields(global_boost_info, fields_in, fields_out)
      eyn(ix) = fields_out(2)
