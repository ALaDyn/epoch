...
CONTAINS
  ...
  SUBROUTINE run_file_injection(injector)
    ...
    INTEGER :: boundary
    REAL(num) :: next_time
    REAL(num) :: vx, gamma, inv_gamma_mass, x_in
    TYPE(particle), POINTER :: new
    TYPE(particle_list) :: plist
    ...
    DO
      ...
      ! Only the rank which is adding the particle can reach here
      ...
      IF (boundary == c_bd_x_min) THEN
        x_in = x_min - (injector%next_time - (time-0.5_num*dt)) * vx
      ELSE IF (boundary == c_bd_x_max) THEN
        x_in = x_max - (injector%next_time - (time-0.5_num*dt)) * vx
      END IF
