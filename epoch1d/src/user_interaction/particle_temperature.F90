...
CONTAINS
  ...
  SUBROUTINE setup_particle_temperature(part_species, partlist_in)

    TYPE(particle_species), POINTER :: part_species
    TYPE(particle_list), POINTER, OPTIONAL :: partlist_in
...
#include "particle_head.inc"
    ...
    IF (PRESENT(partlist_in)) THEN
      partlist => partlist_in
    ELSE
...
#include "particle_to_grid.inc"
 ...
 ! Subroutine to initialise a thermal particle distribution
  SUBROUTINE setup_particle_temperature_relativistic(part_species, partlist_in)
...
#include "particle_to_grid.inc"
      ...
      ENDDO
      current%part_p = &
          momentum_from_temperature_relativistic(mass,temp_local, &
          part_species%fractional_tail_cutoff)
      CALL particle_drift_lorentz_transform(current, mass, drift_local)
      current => current%next
      ipart = ipart + 1
    ENDDO
...
#include "particle_head.inc"
  ...
  !> particle list
  SUBROUTINE sample_partlist_from_distfn(part_species, partlist, iit_r, &
      ipart_r)
